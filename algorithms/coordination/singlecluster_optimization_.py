# -*- coding: utf-8 -*-
"""
Created on Thu Sep 30 07:50:28 2021
...
def decentralized_short_term_rescheduling(clusterdata,connections,solver):

    ###########################################################################
    ################################Data formatting############################
    #clusters =parkdata['clusters']
    horizon  =clusterdata['opt_horizon']
    deltaT   =clusterdata['opt_step']        
    cun_cap  =clusterdata['chrunit_cap']
    cls_cap  =clusterdata['cluster_cap']
    sta_cap  =clusterdata['station_cap']
    ...
    model.p_cc  =Var(model.T,within=NonNegativeReals)                           #Total cluster power  
    ...
    def chargepowerlimit(model,a,t):   #Cars can only be charged until the departure
        if t<departure_time[a]:
            ...
            return model.p_ev[a,t]<=model.P_CU[location[a]]
        else:
    ...
    def ccpower(model,t):
        ...
        return model.p_cc[t]==sum(model.p_ev[a,t] for a in model.A)
    ...
    def cluster_undersizing(model,t):
        #return model.p_cc[c,t]<=model.P_CC[c]
        return model.p_cc[t]<=model.P_CC
    model.ccpowcap =Constraint(model.T,rule=cluster_undersizing)
    ...
    for a in model.A:
        n=location[a]
        opt_ref[n]=model.p_ev[a,min(horizon)]()
