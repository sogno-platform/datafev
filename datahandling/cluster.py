...
class ChargerCluster(object):
    ...
    def __init__(self,cluster_id,topology_data):
        ...
        self.re_dataset=pd.DataFrame(columns=['Active','EV ID','CU ID','Reserved At','From','Until','Cancelled At','Scheduled G2V','Scheduled V2G','Price'])
        ...
        if contract!=None:
            ...
            if contract['Schedule']:
                ...
                if contract['Payment']:
                    ...
                    payment_for_g2v = ((p_ref[p_ref >= 0] * pr_g2v[p_ref >= 0]).sum()) * tdelta/ 3600
                    payment_for_v2g = ((p_ref[p_ref < 0] * pr_v2g[p_ref < 0]).sum()) * tdelta/ 3600

                    self.re_dataset.loc[reservation_id, 'Price']         = payment_for_g2v+payment_for_v2g
    ...
    def enter_data_of_incoming_vehicle(self,ts,ev,cu):
        ...
        self.cc_dataset.loc[cc_dataset_id,'Scheduled G2V [kWh]']=reservation['Scheduled G2V'] if pd.notna(reservation['Scheduled G2V']) else 0
        self.cc_dataset.loc[cc_dataset_id,'Scheduled V2G [kWh]']=reservation['Scheduled V2G'] if pd.notna(reservation['Scheduled V2G']) else 0
    ...
    def query_availability(self,start,end,step):
        ...
        available_chargers=pd.DataFrame(columns=['max p_ch','max p_ds','eff'], dtype=np.float16)
        all_active_reservations=self.re_dataset[self.re_dataset['Active'] == True] #Reservations that are not cancelled

        for cu in self.chargers.values():

            active_reservations = all_active_reservations[all_active_reservations['CU ID']==cu.id]
            ...
            if len(active_reservations) == 0:
                ...
                cu_availability_series = (test_per_reservation.all(axis='columns')).loc[period]

            is_cu_available        =cu_availability_series.all()
            if is_cu_available==True:
                available_chargers.loc[cu.id]={'max p_ch':cu.p_max_ch,'max p_ds':cu.p_max_ds,'eff':cu.eff}
    ...
    def occupation_profile(self,start,end,step):
        df=pd.DataFrame(index=pd.date_range(start=start,end=end,freq=step))
        for cu_id,cu in self.chargers.items():
            df[cu_id]=(cu.occupation_record(start,end,step).reindex(df.index)).fillna(0)      
        return df
    ...
    def export_results(self,start,end,step,xlfile):
        ...
        with pd.ExcelWriter(xlfile) as writer:
            ...
            overall=pd.Series()
            overall['Unfulfilled G2V']=(unfulfilled_g2v_ser[unfulfilled_g2v_ser>0]).sum()
            overall['Unscheduled V2G']=(unscheduled_v2g_ser[unscheduled_v2g_ser>0]).sum()
