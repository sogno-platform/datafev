...
def optimal_intervention_v2g(cs, ts, t_delta, horizon, solver, powlimits):
    opt_horizon_t_steps = pd.date_range(start=ts, end=ts + horizon, freq=t_delta)
    occ = cs.get_cluster_occupations(ts, t_delta, t_delta)
    if occ.iloc[0].sum() > 0:
        ...
        for cc_id, cc in cs.clusters.items():
            for cu_id, cu in cc.cu.items():
                ...
                if connected_car != None:
                    ...
                    connections['battery_cap'][ev_id]    = connected_car.bCapacity
                    connections['departure_time'][ev_id] = (connected_car.estimated_leave-ts)/t_delta
                    connections['initial_soc'][ev_id]    = connected_car.soc[ts]
                    sch_inst = cu.active_schedule_instance
                    ...
                    if cu.ctype=='ac1':
                        p_ev_pos_max=min(connected_car.p_max_ac_ph1,cu.P_max_ch)
                        p_ev_neg_max=min(connected_car.p_max_ac_ph1,cu.P_max_ds) #TODO: Differntiate ev.p_max_ac_ph1_ch/ds
                    if cu.ctype=='ac3':
                        p_ev_pos_max=min(connected_car.p_max_ac_ph3,cu.P_max_ch)
                        p_ev_neg_max=min(connected_car.p_max_ac_ph3,cu.P_max_ds)
                    if cu.ctype=='dc':
                        p_ev_pos_max=min(connected_car.p_max_dc,cu.P_max_ch)
                        p_ev_neg_max=min(connected_car.p_max_dc,cu.P_max_ds)
                    
                    connections['P_EV_pos_max'][ev_id]   =p_ev_pos_max*cu.eff
                    connections['P_EV_neg_max'][ev_id]   =p_ev_neg_max 
                    
        set_points,soc_ref=short_term_rescheduling_bidirectional(parkdata,powlimits,connections,solver)
        for cc_id in cs.clusters.keys():
            for cu_id in cs.clusters[cc_id].cu.keys():
                cu = cs.clusters[cc_id].cu[cu_id]
                if cu.connected_car != None:
