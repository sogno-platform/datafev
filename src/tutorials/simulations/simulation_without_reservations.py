...
from src.datafev.protocols.chargingcontrol.decentralized_fcfs import (
    charging_protocol as charging_protocol_fcs,
)
from src.datafev.protocols.chargingcontrol.decentralized_llf import (
    charging_protocol as charging_protocol_llf,
)
...
for charging_control in ["uncontrolled", "fcfs", "llf"]:
    ...
    system.export_results(
        ...
        sim_end,
        sim_step,
        "result_noreservation_" + charging_control + "_clusters.xlsx",
    )
    fleet.export_results(
        sim_start,
        sim_end,
        sim_step,
        "result_noreservation_" + charging_control + "_fleet.xlsx",
    )
    #######################################################################

    #######################################################################
    # Storing the aggregate consumption profiles of individual clusters to dictionaries
    consumption_profiles_cluster1[charging_control] = (
        cluster1.import_profile(sim_start, sim_end, sim_step)
    ).sum(axis=1)
    consumption_profiles_cluster2[charging_control] = (
        cluster2.import_profile(sim_start, sim_end, sim_step)
    ).sum(axis=1)
    consumption_profiles_cluster3[charging_control] = (
        cluster3.import_profile(sim_start, sim_end, sim_step)
    ).sum(axis=1)
    #######################################################################
...
print()
print(
    "Aggregate consumption profiles of clusters in each management approach are plotted:"
)
