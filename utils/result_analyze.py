...
def print_results(cs, ev_fleet, period, energy_price, xlfile):
    ...
    with pd.ExcelWriter(xlfile) as writer:
        ...
        for cc_id,cc in sorted(cs.clusters.items()):
            ...
            excessive_v2x   = cc_ds['Total V2X [kWh]']-cc_ds['Scheduled V2X [kWh]']
        ...
        energy_summary = pd.DataFrame(columns=['Import (kWh)','Cost of Import (EUR)', 'Cumulative Supply (kWh)','Cumulative V2X (kWh)','Unfulfilled Demand (kWh)', 'Excessive V2X (kWh)'])
        energy_summary['Import (kWh)']            = (pow_to_cc_df * t_delta.seconds / 3600).sum()
        energy_summary['Cost of Import (EUR)']    = ((pow_to_cc_df* t_delta.seconds / 3600).multiply(energy_price/1000,axis='index')).sum()
